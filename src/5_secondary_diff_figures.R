...
df <- res_sec %>% 
  ...
  mutate(contrast = paste0(intervention, " (ref: ", reference,")"), 
         ...
         Y = case_when(
           ...
           Y=="animal.adv.eval_4"~"Animal advocates are misleading",
           Y=="animal.adv.eval_5"~"test",
           Y=="animal.adv.eval_6"~"Animal advocates are accurate",
           Y=="animal.adv.eval_7"~"Animal advocates are similar to me",
...
p
ggsave(p, file=here("figures/unadj/secondary_unadj_diff.png"), width = 10, height=20)
