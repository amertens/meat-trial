...
meat.glm <- function(Yname, Ws=Wvars, data=d, family="gaussian"){
...
glm_mod_format<- function(d=d,Yvar, Wvars=Wvars, family="gaussian", control="worker", contrasts=c("covid", "welfare", "env", "health"), V){
  ...
  for(i in contrasts){
    #print(i)
    res1 <- meat_mod_glm(Y=d[,Yvar], tr=d$tr, W=data.frame(meat_mod=d$meat_mod, W), family=family, contrast=c(control,i), V="meat_mod",  pval=.2)     
    res2 <- meat_mod_glm(Y=d[,Yvar], tr=d$tr, W=data.frame(meat_mod=d$meat_mod, W), family=family, contrast=c(control,i),  pval=.2)  
...
# Y=d[,Yvar]
# tr=d$tr
# W=data.frame(meat_mod=d$meat_mod, W)
...
meat_mod_glm <- function(Y, tr, pair = NULL, W = NULL, forcedW = NULL, V = NULL, 
